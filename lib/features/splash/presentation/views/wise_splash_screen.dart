...
class _ForegroundState extends State<_Foreground> {
  ...
  late final _imageScale = Tween<double>(begin: 1, end: 1.25).chain(CurveTween(curve: Curves.linear)).animate(widget.controller);
...
class _Delegate extends SingleChildLayoutDelegate {
  ...
  bool shouldRelayout(_Delegate oldDelegate) {
    return oldDelegate.progress != progress || oldDelegate.expansion != expansion;
  }
}
...
class _Track {
  ...
  factory _Track.fromScreenRect({
    ...
    final trackWidth = _initialTrackWidth + (screenRect.width - _initialTrackWidth) * expansion;
